...
// --- 兼容性补丁开始 ---
// 低版本系统没有 log 模块，这里进行模拟，防止报错
const LOG_INFO = 6;
const LOG_WARNING = 4;
const LOG_AUTHPRIV = 80; // (10<<3)

function openlog(ident) {
	// 低版本不支持 openlog，这里作为空函数处理
}

function closelog() {
	// 低版本不支持 closelog，这里作为空函数处理
}

function syslog(prio, msg) {
	// 将 syslog 调用重定向到 warn (标准错误输出/logread)
	warn(sprintf("[%s] %s\n", prio, msg));
}
// --- 兼容性补丁结束 ---
