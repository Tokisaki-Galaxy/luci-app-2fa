...
return view.extend({
  load: function () {
    return Promise.all([uci.load("luci"), callListAuthPlugins()]);
  },
  ...
  render: function (data) {
    ...
    m = new form.Map(
      ...
      _(
        "Configure global authentication settings and manage individual authentication plugins.",
      ),
    );
    ...
    s = m.section(
      ...
      _("Global Settings"),
      _(
        "Enable or disable external authentication system-wide. When disabled, only password authentication is used.",
      ),
    );
    s.anonymous = true;
    ...
    o = s.option(
      form.Flag,
      "external_auth",
      _("Enable External Authentication"),
      _(
        "When enabled, authentication plugins in /usr/share/luci/auth.d/ will be loaded and used for additional verification during login.",
      ),
    );
    o.rmempty = false;
    ...
    s = m.section(
      ...
      _(
        "Enable or disable individual authentication plugins. Disabled plugins will be skipped during login.",
      ),
    );
    s.anonymous = true;
    ...
    if (plugins.length > 0) {
      ...
      for (var i = 0; i < plugins.length; i++) {
        ...
        o = s.option(
          ...
          _("Check to disable this plugin. Plugin file: %s").format(
            plugin.filename,
          ),
        );
        o.rmempty = false;
