  ...
  renderWidget: function (section_id, option_id, cfgvalue) {
    ...
    if (!key) {
      containerDiv.appendChild(
        E("em", {}, _("Generate a key first to see the current code")),
      );
      return containerDiv;
    }

    // Time calibration warning div (hidden by default)
    var timeWarningDiv = E(
      "div",
      {
        id: "time-calibration-warning",
        style:
          "display: none; background: #fff3cd; border: 1px solid #ffc107; padding: 10px; border-radius: 4px; margin-bottom: 10px;",
      },
      [
        E(
          "strong",
          { style: "color: #856404;" },
          "⚠️ " + _("Warning: System time not calibrated"),
        ),
        E(
          "p",
          { style: "margin: 5px 0 0 0; color: #856404; font-size: 12px;" },
          _(
            "System time appears to be incorrect (before 2026). TOTP codes will not work during login. Please sync system time.",
          ),
        ),
      ],
    );
      ...
      [
        E(
          "span",
          {
            ...
            style:
              "font-size: 28px; font-weight: bold; font-family: monospace; letter-spacing: 5px; color: #0066cc;",
          },
          _("Loading..."),
        ),
        E(
    ...
    L.Poll.add(function () {
      ...
      return callGetCurrentCode("root")
        .then(function (res) {
          if (res.code && codeEl) {
            ...
            if (
              res.type === "totp" &&
              res.time_remaining !== undefined &&
              countdownEl
            ) {
              countdownEl.textContent = _("Expires in %d seconds").format(
                res.time_remaining,
              );
              ...
              countdownEl.textContent = _("Counter: %s").format(
                res.counter || "0",
              );
              codeEl.style.color = "#0066cc";
            }
  ...
  renderWidget: function (section_id, option_id, cfgvalue) {
    ...
    var clearAllBtn = E(
      "button",
      {
        ...
        click: ui.createHandlerFn(this, function () {
          return callClearAllRateLimits().then(
            function () {
              ui.addNotification(
                null,
                E("p", _("All rate limits cleared.")),
                "info",
              );
              return this.refreshStatus(containerDiv);
            }.bind(this),
    ...
    return E("div", {}, [E("div", {}, [refreshBtn, clearAllBtn]), statusDiv]);
  },

  refreshStatus: function (container) {
    var statusDiv =
      container.querySelector("#rate-limit-status-list") || container;
    statusDiv.innerHTML = "";
    statusDiv.appendChild(E("em", {}, _("Loading...")));

    return callGetRateLimitStatus()
      .then(
        function (result) {
          statusDiv.innerHTML = "";
          ...
          result.entries.forEach(
            function (entry) {
              ...
              var status = entry.locked
                ? _("Locked until ") +
                  new Date(entry.locked_until * 1000).toLocaleString()
                : _("Active");
              ...
              var clearBtn = E(
                "button",
                {
                  ...
                  click: ui.createHandlerFn(
                    this,
                    function () {
                      return callClearRateLimit(entry.ip).then(
                        function () {
                          ui.addNotification(
                            null,
                            E("p", _("Rate limit cleared for ") + entry.ip),
                            "info",
                          );
      ...
      .catch(function (err) {
        ...
        statusDiv.appendChild(
          E(
            "em",
            { style: "color: red;" },
            _("Error loading status: ") + err.message,
          ),
        );
      });
  },
...
return view.extend({
  load: function () {
    return Promise.all([uci.load("2fa"), callGetConfig()]);
  },
  ...
  render: function (data) {
    ...
    m = new form.Map(
      ...
      _(
        "Configure two-factor authentication for LuCI login. " +
    ...
    s = m.section(
      ...
      _("OTP Settings"),
      _(
        "Configure your one-time password settings. Generate a secret key and scan the QR code with your authenticator app.",
      ),
    );
    s.anonymous = true;
    s.addremove = false;
    ...
    o = s.taboption(
      ...
      _("Enable 2FA"),
      _(
        "Enable two-factor authentication for LuCI login. You must configure a secret key before enabling.",
      ),
    );
    o.rmempty = false;
    ...
    o = s.taboption(
      ...
      _("Strict Mode"),
      _(
        "Controls behavior when system time appears uncalibrated (before 2026). " +
          "<br>" +
          _(
    ...
    o = s.taboption(
      ...
      _("Secret Key"),
      _(
        'The secret key used to generate OTP codes. Click "Generate Key" to create a new key, then scan the QR code below with your authenticator app.',
      ),
    );
    ...
    o = s.taboption(
      ...
      _("Current Code"),
      _(
        "This is the current one-time password generated from your secret key. Use it to verify your authenticator app shows the same code.",
      ),
    );
    ...
    o = s.taboption(
      ...
      _("OTP Type"),
      _(
        "TOTP (Time-based) is recommended for most users. HOTP (Counter-based) can be used in environments without reliable time sync.",
      ),
    );
    ...
    o = s.taboption(
      ...
      _("Time Step (seconds)"),
      _(
        "Time interval for TOTP code generation. Default is 30 seconds. Most authenticator apps only support 30 seconds.",
      ),
    );
    o.depends("type", "totp");
    o.default = "30";
    ...
    o = s.taboption(
      ...
      _("Counter"),
      _(
        "Current counter value for HOTP. This increments with each successful login. Only modify if you need to resynchronize with your authenticator.",
      ),
    );
    o.depends("type", "hotp");
    o.default = "0";
    ...
    // ================================================================
    var securitySection = m.section(
      form.NamedSection,
      "settings",
      "settings",
      _("Security Settings"),
      _(
        "Configure security policies to protect your login and manage trusted networks.",
      ),
    );
    ...
    o = securitySection.taboption(
      "whitelist",
      form.Flag,
      "ip_whitelist_enabled",
      _("Enable IP Whitelist"),
      _(
        "Allow specific IP addresses or networks to bypass 2FA authentication. Useful for trusted networks like your home or office LAN.",
      ),
    );
    o.rmempty = false;
    ...
    o = securitySection.taboption(
      ...
      CBIIPWhitelist,
      "ip_whitelist",
      _("Trusted IP Addresses"),
      _(
        "Enter IP addresses or CIDR ranges that should bypass 2FA. These trusted addresses will only need username and password to log in.",
      ),
    );
    ...
    o.cfgvalue = function () {
      return (
        ...
        "<strong>" +
        _("Single IP:") +
        "</strong> 192.168.1.100<br>" +
        "<strong>" +
        _("Subnet:") +
        "</strong> 192.168.1.0/24 (" +
        _("covers 192.168.1.0-255, usable hosts .1-.254") +
        ")<br>" +
        "<strong>" +
        _("Larger network:") +
        "</strong> 10.0.0.0/8<br>" +
        "<strong>" +
        _("IPv6:") +
        "</strong> fd00::/8" +
        "</div>"
    ...
    o = securitySection.taboption(
      ...
      form.Flag,
      "rate_limit_enabled",
      _("Enable Brute Force Protection"),
      _(
        "Protect against automated attacks by temporarily blocking IPs with too many failed login attempts.",
      ),
    );
    o.rmempty = false;
    ...
    o = securitySection.taboption(
      ...
      form.Value,
      "rate_limit_max_attempts",
      _("Max Failed Attempts"),
      _(
        "Number of failed login attempts allowed before an IP is temporarily blocked.",
      ),
    );
    ...
    o = securitySection.taboption(
      ...
      form.Value,
      "rate_limit_window",
      _("Detection Window (seconds)"),
      _(
        "Time period during which failed attempts are counted. After this time, the counter resets.",
      ),
    );
    o.depends("rate_limit_enabled", "1");
    o.default = "60";
    ...
    o = securitySection.taboption(
      ...
      form.Value,
      "rate_limit_lockout",
      _("Lockout Duration (seconds)"),
      _(
        "How long an IP remains blocked after exceeding the max attempts. Default: 5 minutes (300 seconds).",
      ),
    );
    o.depends("rate_limit_enabled", "1");
    o.default = "300";
