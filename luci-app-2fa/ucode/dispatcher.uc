...
// Modified for 2FA support by LuCI-App-2FA Contributors

import { open, stat, glob, lsdir, unlink, basename, popen } from 'fs';
	...
	try {
		...
		if (length(resolved.ctx.auth)) {
			...
			if (!session && resolved.ctx.auth.login) {
				...
				// 2FA verification - check if 2FA is enabled and verify OTP
				if (session && is_2fa_enabled(user)) {
					let otp = http.formvalue('luci_otp');

					if (!otp || otp == '') {
						// OTP not provided, destroy session and show 2FA input page
						...
						http.header('X-LuCI-2FA-Required', 'yes');

						let scope = { duser: user, fuser: null, requires_2fa: true, otp_error: false };
					...
					if (!verify_2fa_otp(user, otp)) {
						// OTP invalid, destroy session and show error
						ubus.call("session", "destroy", { ubus_rpc_session: session.sid });

						openlog('dispatcher.uc');
						syslog(LOG_WARNING|LOG_AUTHPRIV, sprintf("luci: failed 2FA on /%s for %s from %s",
							join('/', resolved.ctx.request_path), user || "?", http.getenv("REMOTE_ADDR") || "?"));
						closelog();
						...
						let scope = { duser: user, fuser: null, requires_2fa: true, otp_error: true };
						let theme_sysauth = `themes/${basename(runtime.env.media)}/sysauth`;
					...
					// OTP valid, log success
					openlog('dispatcher.uc');
					syslog(LOG_INFO|LOG_AUTHPRIV, sprintf("luci: 2FA verified on /%s for %s from %s",
						join('/', resolved.ctx.request_path), user || "?", http.getenv("REMOTE_ADDR") || "?"));
					closelog();
				}
