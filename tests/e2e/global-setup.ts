...
async function globalSetup(config: FullConfig) {
  ...
  for (let i = 0; i < maxRetries; i++) {
    try {
      console.log(
        `  Attempt ${i + 1}/${maxRetries}: Checking ${baseURL}/cgi-bin/luci/`,
      );
      ...
      if (response && (response.ok() || response.status() === 403)) {
        ...
        console.log(
          `  ⚠️  HTTP ${response.status()} but login form not found yet...`,
        );
      } else {
        console.log(
          `  ⚠️  HTTP ${response?.status() || "unknown"} - retrying...`,
        );
      }
    } catch (error: any) {
  ...
  await browser.close();
  throw new Error(
    "❌ LuCI did not become ready in time. Please check if the container is running properly.",
  );
}
