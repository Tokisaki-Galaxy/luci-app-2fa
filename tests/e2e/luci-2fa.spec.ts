...
async function login(
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      throw new Error(
        "Login verification failed - still on login page or admin indicators not found",
      );
    } catch (error) {
      if (attempt === maxRetries - 1) {
        ...
        await page
          .screenshot({
            path: `screenshots/login-failed-attempt-${attempt + 1}.png`,
            fullPage: true,
          })
          .catch(() => {});
...
test.describe("2FA Settings Page", () => {
  test.beforeEach(async ({ page }) => {
    ...
    if (!isLoggedIn) {
      throw new Error(
        "beforeEach: Login verification failed - test cannot proceed",
      );
    }
  });
  ...
  test("should show TOTP-specific options when TOTP is selected", async ({
    page,
  }) => {
    await page.goto("/cgi-bin/luci/admin/system/2fa");
  ...
  test("should show HOTP-specific options when HOTP is selected", async ({
    page,
  }) => {
    await page.goto("/cgi-bin/luci/admin/system/2fa");
...
test.describe("Authentication Settings Page (luci-patch)", () => {
  ...
  test.skip("should navigate to Authentication Settings page", async ({
    page,
  }) => {
    await login(page);
  ...
  test.skip("should display External Authentication toggle", async ({
    page,
  }) => {
    await login(page);
    await page.goto("/cgi-bin/luci/admin/system/admin/authsettings");
