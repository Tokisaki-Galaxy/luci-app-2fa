...
test.describe('2FA Login Flow', () => {
  ...
  test('should successfully login with correct TOTP code', async ({ page }) => {
    ...
    const pageContent = await page.content();
    const hasSession = pageContent.includes('sessionid') && !pageContent.includes('"sessionid": null');
    const isOnAdminPage = currentUrl.includes('admin');
    const hasLogoutOption = pageContent.includes('Logout') || pageContent.includes('logout');
  ...
  test('should reject incorrect TOTP code', async ({ page }) => {
    ...
    const stillOnLoginPage = pageContent.includes('luci_otp') || 
                              pageContent.includes('luci_password');
    
    expect(hasError || stillOnLoginPage).toBeTruthy();
  });

  test('should verify TOTP matches standard implementation', async ({ page }) => {
    ...
    const pageContent = await page.content();
    const loginSucceeded = pageContent.includes('sessionid') && 
