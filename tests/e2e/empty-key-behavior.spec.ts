...
test.describe("Empty Key Behavior - Question 1", () => {
  ...
  test("should allow login with just password when 2FA enabled but key is empty", async ({
    page,
  }) => {
    ...
    const passwordField = page.locator(
      'input[name="luci_password"], #luci_password',
    );
    ...
    if (stillOnLoginPage) {
      ...
      console.log(
        "Still on login page. Page content:",
    ...
    console.log(
      "✅ Test passed: Login succeeded with just password when key is empty",
    );
  });

  test("should show OTP field when 2FA enabled with a valid key", async ({
    page,
  }) => {
